...
import { getProductId, getProductImage, getProductPath } from '../utils/products'
    ...
    <article className="group relative overflow-hidden rounded-[1.1rem] border border-slate-200/90 bg-white transition duration-300 hover:-translate-y-1 hover:border-brand-blue/35 hover:shadow-[0_20px_50px_rgba(23,50,77,0.12)] sm:rounded-none">
      <Link to={productPath} className="block">
        <div className="relative grid h-44 place-items-center overflow-hidden bg-[radial-gradient(circle_at_50%_30%,#fff_0%,#f4fafc_57%,#eaf7fb_100%)] sm:h-64 lg:h-80">
          ...
          {image ? <img src={image} alt={product.name} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" loading="lazy" decoding="async" /> : <div role="img" aria-label={`${product.name} BabyCure product visual`} className="h-full w-full bg-no-repeat transition duration-500 group-hover:scale-105" style={{ backgroundImage: `url(${visual.src})`, backgroundPosition: visual.position, backgroundSize: visual.size }} />}
        </div>
        <div className="px-3 pt-3 sm:px-5 sm:pt-5"><p className="hidden text-[10px] font-extrabold uppercase tracking-[.13em] text-brand-green sm:block">BabyCure essentials</p><h3 className="text-[13px] font-extrabold leading-snug text-brand-ink sm:mt-2 sm:text-[15px]">{product.name}</h3>{product.shortDescription && <p className="mt-1 line-clamp-2 text-xs font-medium leading-5 text-slate-500">{product.shortDescription}</p>}</div>
      </Link>
      <div className="mt-2 flex items-center justify-between gap-2 px-3 sm:mt-3 sm:gap-3 sm:px-5"><div><p className="text-base font-extrabold text-brand-ink sm:text-lg">{formatPrice(product.price)} {mrp > product.price && <span className="ml-1 hidden text-xs font-semibold text-slate-400 line-through sm:inline">{formatPrice(mrp)}</span>}</p></div><Link to={`${productPath}#reviews`} className="hidden rounded-sm bg-[#fff8e7] px-2 py-1 transition hover:bg-amber-100 sm:block" aria-label={`Read or write reviews for ${product.name}`}><Rating rating={rating} reviews={reviews} /></Link></div>
